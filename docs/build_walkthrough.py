    ...
    table(doc,
          ...
          [["Diversity of attacks identified",
           ...
           ["Detection algorithm efficacy",
            "XGBoost + Isolation Forest + entity graph + asymmetric cost/action policy, FPR pinned at 1% on a disjoint split, bootstrap CIs over seeds, precision restated at the 1.3% production base rate",
...
def pillar_identify(doc: Document, cov: dict | None) -> None:
    ...
    if trained and zero_day:
        ...
        if s:
            fpr_mean = num((s.get("legit_fpr", {}) or {}).get("mean"), 3)
            para(doc,
                 ...
                 "supervised training entirely (zero-day). The operating point is pinned at a 1% "
                 "legitimate-FPR budget on a mixed calibration split disjoint from both training "
                 "and evaluation, and is re-derived for every seed and every withheld family. "
                 f"Realised evaluation FPR: {fpr_mean} with a 95% bootstrap CI in the artifact.",
                 space_after=6)
        rows = []
        for fam in trained:
...
def pillar_defend(doc: Document, cal: dict | None, prev: dict | None,
                  econ: dict | None) -> None:
    doc.add_heading("5. Pillar 3 -- Defend: four layers, honestly calibrated", level=1)
    ...
    table(doc,
          ...
          [["Supervised", "XGBoost on behavioural features",
           ...
           ["Action policy", "Asymmetric cost/action ladder with INR-priced false-positive and insult costs",
            "Turns scores into issuer actions. Keeps a 1% FPR affordable in production by reserving outright declines for high-cost, high-confidence alerts."]],
    ...
    para(doc,
         ...
         "Where a figure is produced by a full-stack operating-point search rather than a "
         "single supervised threshold, the calibration split carries a mixed stream and the "
         "realised evaluation FPR is reported alongside the target, because a pinned budget is "
         "a design constraint rather than a guarantee. "
...
def feasibility(doc: Document, lat: dict | None) -> None:
    ...
    for t in [
        ...
        "The four-layer stack is a working prototype, not a hardened deployment: "
